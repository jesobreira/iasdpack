...
$Form1 = GUICreate("Escala da Sonoplastia - IASD Nova Almeida", 515, 290, 199, 231)
...
$Group1 = GUICtrlCreateGroup(" Escalado ", 184, 8, 321, 49)
$iEscalado = GUICtrlCreateInput("", 192, 24, 305, 21)
GUICtrlSetOnEvent(-1, "iEscaladoChange")
GUICtrlCreateGroup("", -99, -99, 1, 1)
$Group2 = GUICtrlCreateGroup(" Aprendiz/auxiliar ", 184, 64, 321, 49)
$iAprendiz = GUICtrlCreateInput("", 192, 80, 305, 21)
GUICtrlSetOnEvent(-1, "iAprendizChange")
GUICtrlCreateGroup("", -99, -99, 1, 1)
$Group4 = GUICtrlCreateGroup(" Telefone do Sonoplasta ", 8, 176, 169, 105)
$iSonoplastaTel = GUICtrlCreateCombo("Selecione um sonoplasta...", 16, 192, 153, 25)
...
$iJA = GUICtrlCreateCheckbox("Culto Jovem", 184, 232, 81, 25)
...
Func showescala($data, $ja)
	...
	GUICtrlSetData($iEmnomede, db($data, "visto"))
EndFunc
...
Func __generate($month, $year=@YEAR)
	$html = '<!DOCTYPE html>' & @CRLF & '<html>' & @CRLF & '<head>' & @CRLF & '	<meta charset="utf-8" />' & @CRLF & '	<title>Escala de Sonoplastia - %MES% de %ANO%</title>' & @CRLF
	...
	$html &= @CRLF & '	<h1>Escala de Sonoplastia - ' & mes($month) &' de ' & $year & '</h1>' & @CRLF
	
	$html &= '	<table>' & @CRLF & '		<tr>' & @CRLF & '			<th>' & @CRLF & '				Dia' & @CRLF & '			</th>' & @CRLF & '			<th>' & @CRLF & '				Escalado' & @CRLF & '			</th>' & @CRLF & '			<th>' & @CRLF & '				Auxiliar' & @CRLF & '			</th>' & @CRLF & '			<th>' & @CRLF & '				Obs.' & @CRLF & '			</th>' & @CRLF & '		</tr>' & @CRLF
